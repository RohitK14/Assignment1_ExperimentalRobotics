...
class Normal(smach.State):
    ...
    def __init__(self):
        smach.State.__init__(self, 
                            ...
                            input_keys=['normal_tired_counter_in', 'normal_pos_person_in'],
                            output_keys=['normal_tired_counter_out', 'normal_pos_person_out'])
        self.pub = rospy.Publisher('/moveToPose', Point, queue_size=10)
        self.isReached = rospy.Subscriber('/Reached',Bool,reachCallback)
        self.command_play = rospy.Subscriber('/command', String, commandCallback)
    ...
    with sm:
        ...
        smach.StateMachine.add('NORMAL', Normal(), 
                               ...
                               remapping={'normal_tired_counter_in':'tireness',
                                          ...
                                          'normal_pos_person_in':'person',
                                          'normal_pos_person_out':'person'})
        smach.StateMachine.add('SLEEP', Sleep(), 
                               transitions={'wake_up':'NORMAL'},
                               remapping={'sleep_timer_in':'tireness',
                                          'sleep_timer_out':'tireness'})

        smach.StateMachine.add('PLAY', Play(), 
                               transitions={'go_to_normal':'NORMAL'},
                               remapping={'play_timer_in':'tireness',
                                          'play_timer_out':'tireness'})
